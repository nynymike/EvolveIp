...
var passportSamlProxy = require('../auth/saml_proxy').passport;
...
var callbackResponse = function(req, res) {
    ...
    logger.log('info', '--------------------GOT PASSPORT JS CALL BACK RESPONSE-------');
...
//==================== twitter =================
router.post('/auth/twitter/callback',
...
//================== saml-proxy ==================
router.post('/auth/saml_proxy/callback/:config',
    function(req,res,next){

        logger.log('info', '-----------------------------------------------------------');
        logger.log('info', '------------------CALL BACK SAML-PROXY------------------------------');
        logger.log('info', '-----------------------------------------------------------');
        logger.log('info', 'CONFIG==>'+req.params.config);
        //logger.log('info', 'STATE==>'+req.params.state);
        logger.log('info', '-----------------------------------------------------------');

        passportSamlProxy.authenticate(req.params.config,
            {
                failureRedirect: '/passport/login',
                failureFlash: true
            })(req,res,next)
...
router.get('/auth/saml_proxy/:token/config/:config',
    //validateToken,
    function(req,res,next){

        logger.log('info', '-----------------------------------------------------------');
        logger.log('info', '------------------ENTRY POINT SAML-PROXY------------------------------');
        logger.log('info', '-----------------------------------------------------------');
        logger.log('info', 'CONFIG==>'+req.params.config);
        //logger.log('info', 'STATE==>'+req.params.state);
        logger.log('info', '-----------------------------------------------------------');
